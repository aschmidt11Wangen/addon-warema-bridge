...
function privateCmdQueueEnqueue( stickObj, wmsMsg, onEnd, priority ){
	...
	if( priority === "priority" ){
		...
		log.I( "Enqueued (priotity): "+wmsMsg.msgType+" "+wmsMsg.snr+" params: "+JSON.stringify(wmsMsg.params) );
	}
	else{
		stickObj.wmsMsgQueue.push( wmsMsg );
		log.I( "Enqueued: "+wmsMsg.msgType+" "+wmsMsg.snr+" params: "+JSON.stringify(wmsMsg.params) );
	}
}
	...
	if( options ){
		if( options.autoAssignBlinds ){
			...
			stickObj.scannedDevArray.forEach( function (device, index) { 
					if( ( device.type === '20' ) || ( device.type === '21' ) || ( device.type === '25' )|| ( device.type === '2E' ) ){
...
class WmsVbStick {
  ...
  constructor( name, channel, panid, key, optionsPar, callback, comDataSendCallback ){
	...
	vnBlindAdd( snr, name ){
		...
		var blind = {};
		blind.snr = wmsUtil.snrHexToNum( snr );
