			...
			ports => { 
				...
				ports.forEach((port) => {
					...
					if( true || port.path.includes( "USB" ) ){
						...
						var testPort = new SerialPort({ path: port.path, baudRate: 125000 })
						var testParser = testPort.pipe(new DelimiterParser({ delimiter: DelimiterChar }))
